...
import { getUsersApi, createUserApi } from '../services/userService';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onIncidentClick, refreshKey }) => {
    ...
    const breachedIncidents = incidents.filter(i => {
        ...
        return !isClosed && now > deadline;
    }).length;
    const withinSLA = totalIncidents - breachedIncidents;

    // Charts Data
    const priorityCounts = { Critical: 0, High: 0, Medium: 0, Low: 0 };
    incidents.forEach(i => { if (priorityCounts[i.priority] !== undefined) priorityCounts[i.priority]++; });
    ...
    incidents.forEach(i => {
        const statusKey = i.status.replace(' ', '') as keyof typeof statusCounts;
    ...
    const dynamicCategoryData = Object.keys(categoryCounts).map(key => ({
        name: key,
    ...
    const dynamicActivityData = last7Days.map(day => ({
        ...
        incidents: activityMap[day]
    }));



    // Load incidents for Operations tab
    // Load incidents and users
    useEffect(() => {
    ...
    const handleCreateUser = async (e: React.FormEvent) => {
        ...
        try {
            const newUserPayload = {
                email: newUser.email,
                name: newUser.name, // The backend might expect 'name' or just email/role. Adjust if needed.
                role: newUser.role,
                password: 'password123' // Default pass for new users as per demo
            };
            const created = await createUserApi(newUserPayload);
            ...
            {activeTab === 'analytics' && (
                <div className="animate-slide-up space-y-6">
                                ...
                                {[
                                    ...
                                    <div key={idx} className="flex-1 flex flex-col items-center group">
                                        <div className="w-full bg-gray-100 dark:bg-dark-700 rounded-t-lg relative overflow-hidden h-full flex items-end">
                                            <div
                                                className={`w - full transition - all duration - 1000 ${item.s >= 4 ? 'bg-green-500' :
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <DashboardCard
                            label="Total Incidents" value={totalIncidents.toString()} sub="All time"
                            icon={<FileText size={20} />} color="text-gray-600 dark:text-gray-300" bg="bg-gray-100 dark:bg-dark-700"
                            trend="Live" trendColor="text-green-500"
                        />
                        <DashboardCard
                            label="Open" value={openIncidents.toString()} sub="Requires attention"
                            icon={<Clock size={20} />} color="text-amber-600 dark:text-amber-500" bg="bg-amber-100 dark:bg-amber-900/20"
                            trend={`${((openIncidents / totalIncidents || 0) * 100).toFixed(0)}%`} trendColor="text-amber-500"
                        />
                        <DashboardCard
                            label="Resolved" value={resolvedIncidents.toString()} sub="Resolution rate"
                            icon={<CheckCircle size={20} />} color="text-green-600 dark:text-green-500" bg="bg-green-100 dark:bg-green-900/20"
                            trend={`${resolutionRate}%`} trendColor="text-green-500"
                        />
                        <DashboardCard
                            label="SLA Breached" value={breachedIncidents.toString()} sub="Critical"
                            icon={<AlertTriangle size={20} />} color="text-red-600 dark:text-red-500" bg="bg-red-100 dark:bg-red-900/20"
                            trend="Action Needed" trendColor="text-red-500"
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white dark:bg-dark-800 p-6 rounded-xl border border-gray-200 dark:border-dark-700 shadow-sm">
                            ...
                            <div className="h-64">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={dynamicPriorityData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                        ...
                        <div className="bg-white dark:bg-dark-800 p-6 rounded-xl border border-gray-200 dark:border-dark-700 shadow-sm">
                            ...
                            <div className="h-64">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={dynamicCategoryData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                                        <CartesianGrid strokeDasharray="3 3" horizontal={false} opacity={0.1} />
                                        <XAxis type="number" stroke="#888" fontSize={10} />
                                        <YAxis dataKey="name" type="category" stroke="#888" fontSize={11} width={60} />
                                        <Tooltip
                                    ...
                                    {incidents.map((inc) => (
                                        <tr key={inc.id} className="hover:bg-gray-50 dark:hover:bg-dark-700/50 transition cursor-pointer" onClick={() => onIncidentClick(inc.id)}>
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px - 2 py - 1 rounded text - xs font - medium ${inc.priority === 'Critical' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400' :
                                ...
                                <tbody className="divide-y divide-gray-100 dark:divide-dark-700">
                                    {users.map(u => (
                                        <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-dark-700/50 transition">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px - 2 py - 1 rounded text - xs font - medium border ${u.role === 'Admin' ? 'bg-red-50 text-red-600 border-red-200 dark:bg-red-900/10 dark:text-red-400 dark:border-red-900/30' :
...
const TabButton: React.FC<{ active: boolean, onClick: () => void, icon: React.ReactNode, label: string }> = ({ active, onClick, icon, label }) => (
    <button
        onClick={onClick}
        className={`flex - none md: flex - 1 flex items - center justify - center py - 3 px - 6 text - base font - semibold rounded - lg transition - all whitespace - nowrap ${active
