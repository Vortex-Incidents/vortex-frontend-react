    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                // Ensure dates are parsed if API returns strings
                const parsedData = data.map(i => ({
                    ...i,
                    createdAt: new Date(i.createdAt),
                    slaDeadline: new Date(i.slaDeadline)
                }));
                                ...
                                return (
                                    <tr key={inc.id} onClick={() => onIncidentClick(inc.id)} className="hover:bg-gray-50 dark:hover:bg-dark-700/50 transition cursor-pointer">
                                        <td className="px-6 py-4">
                                            <div className={`w-3 h-3 rounded-full ${isCompleted ? 'bg-gray-300 dark:bg-gray-600' :
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${inc.status === 'Resolved' ? 'bg-green-100 text-green-800' :
