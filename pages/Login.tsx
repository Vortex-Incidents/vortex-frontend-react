...
const Login: React.FC<LoginProps> = ({ onLogin, onBack }) => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Token is already stored by loginApi, but we ensure state update
            onLogin(role);
        } catch (error) {
...
const RoleButton: React.FC<{ role: Role, current: Role, onClick: () => void, icon: React.ReactNode }> = ({ role, current, onClick, icon }) => (
    <button
        type="button"
        onClick={onClick}
        className={`flex flex-col items-center justify-center p-2 rounded-lg border transition-all duration-200 ${current === role
